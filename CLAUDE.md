...
- **The TYPE stamps the policy, so it cannot be mis-authored.** Four behaviours are not authorable and no file may set them: a contract is never handed out or tracked on its own (it exists because a board posted it), it stays out of the open quest log until taken and never occupies a log slot, its repeat is governed by whatever posted it and clocked from FINISHING, and it is handed in at the board that posted it. The one payout choice the file DOES make is `Auto` against `Claim`; park it under `Claim` so a reward survives the board turning over. This is why a contract file is four to six lines with no lifecycle block.
...
- **Every requirement is a factor bound.** `Requires` is the same shared block a quest carries: `Factors` (numeric bounds on the shared factor vocabulary), `Permission`, `Quests`, `Custom`, and `AllOf`/`AnyOf`/`Not` for combinations (a `Not` group is one that must FAIL, the way to say "only while this is not yet true"). The MMO answers `hytale:stat` for its own channels, so `MMO_Level_MINING` and `MMO_CombatLevel` are just parameters, and a pack can mint its own requirement word with a `Server/ZiggfreedCommon/Factors/<Id>.json` file and no code at all.
...
- **No step wording is needed.** The renderer builds a localized "Defeat 12 Zombie" from the step itself, in every language; the target's own name reads in the singular, because nothing pluralizes at runtime. Author `TextKey` on a step only when the generated line would read badly.
...
3. **Validate:** `/mmobounty validate` (console/admin). Confirm no `EMPTY_BOARD`, `UNFILLABLE_SLOT`, `UNKNOWN_BOARD`, `ORPHANED_BOUNTY` or `MISSING_REROLL_CURRENCY` findings for the new board.
...
- **Icons come free where the payout implies one**: with no authored `Icon`, an offer takes its art from its first reward, so a `Currency` reward shows the wallet's icon, a `Command` `/give` the given item, and an `Mmo_Xp` reward for a concrete skill that skill's registry icon. An authored `Icon` always wins, and it INHERITS: `Xp_Packet` authors the purple crystal, so every generated packet wears it. Drop the `Icon` from the base if you would rather each packet show its own skill.
- **Validate:** `/mmoshop validate` reports a missing or disabled cost currency, a non-positive cost, an unknown reward kind, an empty rewards list, an offer naming no storefront, an unknown `Pool.Id`, and (for shelves) empty pools, unfillable slots and oversubscribed pools.
...
- **Override one generated packet** by authoring an explicit `ShopEntries` file with that generated id. An authored file beats a generated offer of the same id outright, and the fold reports the collision so a too-coarse `IdPattern` is never mistaken for a deliberate special case. There is no owner file over offers, so this is the one route.
...
- Owner overrides live in `mods/ziggfreedcommon/currencies.json`, with `shops.json`, `shop-pools.json` and `boards.json` beside it for the storefronts, the shelves and the boards.

## Bounty achievements

The mod ships **board-agnostic** bounty achievements in its own jar (a count ladder, a hard-difficulty chain, a server-first, a daily streak), driven by the `COMPLETE_BOUNTY` / `BOUNTY_STREAK` moments fired on every completion, so they work with any board including a server's own. This pack adds **per-board** chains (`Daily Contractor`, `Weekly Warrant`, `Quick Contractor`) plus the Snapdragon creature chain, since those board ids only exist when this pack is installed. They live at `Server/ZiggfreedCommon/Achievements/MMOSkillTree/Bounty/<Name>.json`, where the FILE NAME is the achievement id: `Criteria: {"complete-bounty-daily": {"Kind": "COMPLETE_BOUNTY", "Target": "<board>", "MatchMode": "EXACT", "Amount": N}}` (criteria are keyed by criterion id, which is also what progress is stored under, so a later tier can retune one criterion by name), the pay under `Rewards: {"Claim": [...]}` (`Auto` pays the instant it is earned, `Claim` waits on the achievements surface), the ladder on `Listing.Chains: [{"Id": "bounty_board_daily", "Tier": 2}]`, and the shared description line's number from `Text.TextArgs.Flavor: ["@amount"]` so all three rungs translate once. The completion moment carries `target = board id` and `qualifier = difficulty`, so an achievement can match by board, by difficulty, or both. A new board's chain is a copy with the board id and the amounts changed.
