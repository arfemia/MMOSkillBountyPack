...
A standalone Hytale content pack for the [MMOSkillTree mod](https://www.curseforge.com/hytale/mods/mmo-skill-tree) (1.4.0+; the manifest hard-deps `^1.4.0` since the Dailies / Bihourly boards use 1.4.0 engine features). Ships the entire **Bounty Board** feature's content: the bounty pool, reusable templates, the Bounty Token currency, the board schedules, AND the in-world board blocks (item + interaction + textures-via-vanilla + item names). The mod jar ships only the Bounty Board *engine* (`BountyService`, `BountyBoardConfig`, `BountyBoardPage` + its `.ui`, the registered `mmo_bounty_board_open` interaction type, `/mmobounty*` commands) and no content, so this pack is what makes bounties appear. It is a **hard dependency** on the mod (the board block's interaction needs the jar-registered Java type), declared in `manifest.json`.
...
└── Server/
    ...
    └── MMOSkillTree/
        ...
        ├── Quests/Bounty_*.json                        the bounty pool (daily + weekly + dailies + bihourly; kill / gather / train-skill / deliver / place / fish / harvest / spend)
        ...
        ├── BountyBoards/Daily.json, Weekly.json, Dailies.json, Bihourly.json   board schedules (rotation/selection/slots/combat gate)
...
- **The shipped boards (1.4.0).** Four boards ship: the rotating `Daily` + `Weekly` token faucets (weighted_random, combat-gated, full token payouts), plus two XP-focused boards migrated from the old jar dailies. `Dailies` (id `dailies`) uses **selection `{"type": "all"}`** (the whole `board:dailies` pool is active every day, `slots`/`rerollCost` omitted) and `Bihourly` (id `bihourly`) uses a generic **`"period": "7200s"`** with a single easy slot and a token-cost reroll. **Deliberate economy rule: the Dailies/Bihourly *quests* award minimal-to-no bounty tokens** (Dailies a small nominal token + flat XP, Bihourly **pure flat XP, no token**) - they are XP-grind content, not a token source. The token faucet is the `Daily`/`Weekly` boards alone; do NOT "fix" the dailies/bihourly back to full token rewards (it would inflate the economy, since `all` runs the whole Dailies pool daily and `bihourly` rotates several times a day). The `bihourly` reroll still costs tokens, so it stays a small sink on free-XP contracts.
